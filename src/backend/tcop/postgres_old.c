...
/* ----------------------------------------------------------------
 *		code for hw_compiler in this file
 * ----------------------------------------------------------------
 */

/* ----------------------------------------------------------------
 *		#define for hw_compiler in this file
 * ----------------------------------------------------------------
 */

#define ENUM_NONE					0
#define ENUM_SELECT					1
#define ENUM_LOGREGR_PREDICT		2
#define ENUM_LINREGR_PREDICT		3
#define ENUM_SVM_PREDICT			4
#define ENUM_MLP_PREDICT			5
#define ENUM_FROM					6

/* ----------------------------------------------------------------
 *		struct datatype for hw_compiler in this file
 * ----------------------------------------------------------------
 */
struct Logregr_op {
	int word_start;
...
/* ----------------------------------------------------------------
 *		func for hw_compiler in this file
 * ----------------------------------------------------------------
 */
static int
...
static int
hw_hashmap(char* str) {
	if(hw_strcmp(str, "SELECT"))						return ENUM_SELECT;
	else if(hw_strcmp(str, "madlib.logregr_predict"))	return ENUM_LOGREGR_PREDICT;
	else if(hw_strcmp(str, "madlib.linregr_predict"))	return ENUM_LINREGR_PREDICT;
	else if(hw_strcmp(str, "madlib.svm_predict"))		return ENUM_SVM_PREDICT;
	else if(hw_strcmp(str, "madlib.mlp_predict"))		return ENUM_MLP_PREDICT;
	else if(hw_strcmp(str, "FROM"))						return ENUM_FROM;
	else												return ENUM_NONE;
}

static void
hw_word_division(const char* query_string, char** word_array, char* str) {

	strcpy(str, query_string);

	char* ptr_temp;
	int size = 0;
	ptr_temp = strtok(str, " ,()[]\n;'");
	while(ptr_temp != NULL) {
		word_array[size] = ptr_temp;
		size++;
...
static void
hw_none_opcheck(int* class) {
	switch(*class) {
		case ENUM_NONE:
			break;
		case ENUM_SELECT:
			break;
		case ENUM_LOGREGR_PREDICT:
			logregr_op.word_end++;
			break;
		case ENUM_LINREGR_PREDICT:
			linregr_op.word_end++;
			break;
		case ENUM_SVM_PREDICT:
			svm_op.word_end++;
			break;
		case ENUM_MLP_PREDICT:
			mlp_op.word_end++;
			break;
		case ENUM_FROM:
			break;
...
static void
hw_op_check(char** word_array, int word_size) {
	int idx = 0;
	int class = 0;
	char* word = word_array[idx];
	while(idx < word_size) {
		switch(hw_hashmap(word)) {
			case ENUM_NONE:
				hw_none_opcheck(&class);
				break;
			case ENUM_SELECT:
				class = ENUM_SELECT;
				break;
			case ENUM_LOGREGR_PREDICT:
				class = ENUM_LOGREGR_PREDICT;
				logregr_op.exist = true;
				logregr_op.word_start = idx;
				logregr_op.word_end = idx;
				break;
			case ENUM_LINREGR_PREDICT:
				class = ENUM_LINREGR_PREDICT;
				linregr_op.exist = true;
				linregr_op.word_start = idx;
				linregr_op.word_end = idx;
				break;
			case ENUM_SVM_PREDICT:
				class = ENUM_SVM_PREDICT;
				svm_op.exist = true;
				svm_op.word_start = idx;
				svm_op.word_end = idx;
				break;
			case ENUM_MLP_PREDICT:
				class = ENUM_MLP_PREDICT;
				mlp_op.exist = true;
				mlp_op.word_start = idx;
				mlp_op.word_end = idx;
				break;
			case ENUM_FROM:
				class = ENUM_FROM;
...
static void
hw_query_modification(const char* query_string, const char* query_string_new, char* str2, char** word_array) {
	// Only for svm_predict function (avoid madlib error)
	if(svm_op.exist && hw_strcmp(word_array[0], "SELECT")) {	
		strcpy(str2, query_string);
		strcpy(query_string_new, "SELECT * FROM ");
		int i = 0;
		char* table_tmp;
		table_tmp = strtok(str2, " ,()[]\n;'");
		while(table_tmp != NULL) {
			if(i==2) {
				strcat(query_string_new, table_tmp);
				strcat(query_string_new, ", ");
			}
			if(i==3) {
				strcat(query_string_new, table_tmp);
				strcat(query_string_new, ";");
			}
			table_tmp = strtok(NULL, " ,()[]\n;'");
			i++;
		}
	}
	// Only for mlp_predict function (avoid madlib error)
	if(mlp_op.exist && hw_strcmp(word_array[0], "SELECT")) {
		strcpy(str2, query_string);
		strcpy(query_string_new, "SELECT * FROM ");
		int i =0;
		char* table_tmp;
...
static void
hw_op_define(int rtable_num, int* rtable_id, int* rtable_relid, int*rtable_colnum, char*** rtable_colname, char** word_array) {
	//logistic regression
	if(logregr_op.exist) {
		bool operand_flag = 1; // 0:coef, 1:data
		bool find_flag = 0;
		int idx = 0;
		int attr_size = logregr_op.word_end-logregr_op.word_start;
		while(idx<attr_size) {
			char* attr = word_array[logregr_op.word_start+idx+1];
			for(int i=0; i<rtable_num; i++) {
				for(int j=0; j<rtable_colnum[i]; j++) {
					if(strcasecmp(attr, rtable_colname[i][j]) == 0) {
						if(!operand_flag) {
							logregr_op.coef_table_id = i;
							logregr_op.coef_table_relid = rtable_relid[i];
							logregr_op.coef_colbitmap += pow(2,j);
						}
						else {
							logregr_op.data_table_id = i;
							logregr_op.data_table_relid = rtable_relid[i];
							logregr_op.data_colbitmap += pow(2,j);
						}
						find_flag = 1;
					}
				}
	...
	if(linregr_op.exist) {
		...
		bool find_flag = 0;
		int idx = 0;
		int attr_size = linregr_op.word_end-linregr_op.word_start;
		while(idx<attr_size) {
			char* attr = word_array[linregr_op.word_start+idx+1];
			for(int i=0; i<rtable_num; i++) {
				for(int j=0; j<rtable_colnum[i]; j++) {
					if(strcasecmp(attr, rtable_colname[i][j]) == 0) {
						if(!operand_flag) {
							linregr_op.coef_table_id = i;
							linregr_op.coef_table_relid = rtable_relid[i];
							linregr_op.coef_colbitmap += pow(2,j);
						}
						else {
							linregr_op.data_table_id = i;
							linregr_op.data_table_relid = rtable_relid[i];
							linregr_op.data_colbitmap += pow(2,j);
						}
						find_flag = 1;
					}
				}
	...
	//svm
	if(svm_op.exist) {
		int numfeature = rtable_colnum[1]-2; //index, label

		svm_op.coef_table_id = 0;
		svm_op.coef_table_relid = rtable_relid[svm_op.coef_table_id];

		svm_op.data_table_id = 1;
		svm_op.data_table_relid = rtable_relid[svm_op.data_table_id];

		for(int i=0; i<numfeature; i++) {
			svm_op.coef_colbitmap += pow(2,i);
		}
		for(int i=2; i<rtable_colnum[1]; i++) {
			svm_op.data_colbitmap += pow(2,i);
		}
	}
	//mlp
	if(mlp_op.exist) {
		mlp_op.coef_table_id = 0;
		mlp_op.coef_table_relid = rtable_relid[mlp_op.coef_table_id];
		mlp_op.coef_colsel = rtable_colnum[0]-3; //loss, num_iterations, coeff

		mlp_op.data_table_id = 1;
		mlp_op.data_table_relid = rtable_relid[mlp_op.data_table_id];

		for(int i=2; i<rtable_colnum[1]; i++) {
			mlp_op.data_colbitmap += pow(2,i);
		}
	}
}

static void
hw_rtable_extract(Query* query, int* rtable_id, int* rtable_relid, char** rtable_name, int* rtable_colnum, char*** rtable_colname) {
	...
	foreach(rtable_list, query->rtable) {
		...
		//rtable_colsel[idx] = (rtbl->selectedCols->words)[0];

		int col_num = list_length(rtbl->eref->colnames);
		/*
		ListCell* col_list;
		foreach(col_list, rtbl->eref->colnames) {
			col_num++;
		}
		*/
...
static void
hw_static_analyzer(const char* query_string) {
	int query_length = strlen(query_string);
	char* str0 = (char*)malloc(sizeof(char)*(query_length+1));
...
static void
hw_compiler(const char* query_string, List* querytrees) {

	int query_length = strlen(query_string);
	char* str0 = (char*)malloc(sizeof(char)*(query_length+1));
	strcpy(str0, query_string);

	int word_size = 0;
	char* word_tmp;
	word_tmp = strtok(str0, " ,()[]\n;'");
	while(word_tmp != NULL) {
		word_size++;
		word_tmp = strtok(NULL, " ,()[]\n;'");
	}

	char** word_array = (char**)malloc(sizeof(char*)*(word_size));
	char* str1 = (char*)malloc(sizeof(char)*(query_length+1));
	hw_word_division(query_string, word_array, str1);
	hw_op_check(word_array, word_size);

	char* str2 = (char*)malloc(sizeof(char)*(query_length+1));
	const char* query_string_new = (char*)malloc(sizeof(char)*(query_length+19+1));

	hw_query_modification(query_string, query_string_new, str2, word_array);
	
	List* hw_parsetree_list;
	ListCell* hw_parsetree_item;
	List* hw_querytree_list;

	//Query for non SQL form
	if(svm_op.exist && hw_strcmp(word_array[0], "SELECT")) {
		hw_parsetree_list = pg_parse_query(query_string_new);

		foreach(hw_parsetree_item, hw_parsetree_list)
		{
			RawStmt* hw_parsetree = lfirst_node(RawStmt, hw_parsetree_item);
			hw_querytree_list = pg_analyze_and_rewrite(hw_parsetree, query_string_new, NULL, 0, NULL);
		}
	}
	if(mlp_op.exist && hw_strcmp(word_array[0], "SELECT")) {
		hw_parsetree_list = pg_parse_query(query_string_new);
	...
	if(svm_op.exist && hw_strcmp(word_array[0], "SELECT"))		hw_querytrees = hw_querytree_list;
	else if(mlp_op.exist && hw_strcmp(word_array[0], "SELECT")) hw_querytrees = hw_querytree_list;
	else														hw_querytrees = querytrees;

	foreach(query_list, hw_querytrees) {
		...
		*/
		...
		hw_rtable_extract(query, rtable_id, rtable_relid, rtable_name, rtable_colnum, rtable_colname);
		hw_op_define(rtable_num, rtable_id, rtable_relid, rtable_colnum, rtable_colname, word_array);
	}
...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		hw_compiler(query_string, querytree_list);
			...
			case 'Q':			/* simple query */
				{
					...
					hw_static_analyzer(query_string);
